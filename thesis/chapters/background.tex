...
The path integral is an alternative representation of the rendering equation that comprehensively explains specific rendering aspects.
Before introducing the equation, one will examine the benefits.
...
The following equation \ref{eq:path-integral} represents the path integral.
It summates the radiance of all contributing paths, which delivers the value received by the camera plane.
...
Consequentially, eq.~\ref{eq:path-radiance} is the amout of radiance of a path $\bar{x}_n=x_0,x_1,\dotso,x_n$ with $n+1$ vertices.
...
As explained by \cite{pharr_physically_2017}, the throughput describes the fraction of received radiance from a light source.
...
The geometric term, see eq.~\ref{eq:path-geometric}, represents the geometric configuration of two related vertices.
That is due to the prior projected solid angle in eq.~\ref{eq:rendering}, which enables one to change to a point-based area-related meaning.

\begin{align}
G(x_{k+1}\leftrightarrow x_{k})=V(x_{k+1}\leftrightarrow x_{k})\frac{\left|\cos\theta_{k+1}\right|\left|\cos\theta_k\right|}{\left\|x_{k+1}\rightarrow x_k\right\|^2}
\label{eq:path-geometric}
\end{align}

$V(x_{k+1}\leftrightarrow x_{k})$ is a visibility term that yields one if two points are mutually visible; otherwise, zero.
